...
exports.cleanupPresence = onSchedule(
  ...
  async () => {
    ...
    const snap   = await db.collection('active_sessions')
...
/**
 * Triggered when a join request is written to the joinRequests collection.
 * Sends an FCM push to the target player's device.
 */
exports.onJoinRequest = onDocumentCreated(
  ...
  async (event) => {
    ...
    const fromName   = data.fromDisplayName ?? 'A golfer';
    const courseName = data.courseName ?? 'the course';

    logger.info(`[joinRequest] ${fromName} → uid=${toUserId} at ${courseName}`);

    const recipientDoc = await admin.firestore().collection('users').doc(toUserId).get();
    const token = recipientDoc.data()?.fcmToken;

    if (!token) {
      logger.warn(`[joinRequest] No fcmToken for uid=${toUserId} — skipping`);
      return;
    }
    ...
    try {
      await admin.messaging().send({
        ...
        notification: {
          title: '⛳ Join Request',
          body: `${fromName} wants to join your group at ${courseName}`,
        },
        data: {
          route: 'joinRequest',
      ...
      logger.info(`[joinRequest] FCM sent to uid=${toUserId}`);
    } catch (err) {
      logger.error(`[joinRequest] FCM failed for uid=${toUserId}: ${err.message}`);
    }
  },
);
